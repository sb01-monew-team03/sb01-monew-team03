...
ALTER TABLE articles
    ADD CONSTRAINT uq_articles_original_link
        UNIQUE (original_link);

ALTER TABLE articles
    ADD CONSTRAINT chk_articles_source_not_empty
        CHECK (length(trim(source)) > 0);

ALTER TABLE articles
    ADD CONSTRAINT chk_articles_title_not_empty
...
ALTER TABLE articles
    ADD CONSTRAINT chk_articles_updated
...
-- 필요시 업데이트 시간 제약조건 추가
ALTER TABLE activity
    ADD CONSTRAINT chk_activity_updated
...
ALTER TABLE notification
    ADD CONSTRAINT fk_notification_users
        FOREIGN KEY (user_id) REFERENCES users (id) ON DELETE CASCADE;
